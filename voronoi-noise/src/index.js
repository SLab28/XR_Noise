...
let camera = createCamera(90, 0.01, 1000, { x: 0, y: 1.8, z: 4 })
...
let app = {
  async initScene() {
    ...
    // Prepare AR sizing and interaction state
    this.geometry.computeBoundingSphere()
    this.particlesRadius = (this.geometry.boundingSphere && this.geometry.boundingSphere.radius) || 1.0
    this.xrMode = null
    this.arZoomFactor = 1.0
    this._pinch = { active: false, startDist: 0, startZoom: 1.0 }
    
    // AR sphere center position in world coordinates (meters)
    // Adjust these values to position the sphere relative to the AR room
    this.arSphereCenter = {
      x: 0,      // left/right (negative = left, positive = right)
      y: 0,      // up/down (negative = down, positive = up) 
      z: -3.5    // forward/back (negative = further back, positive = closer)
    }

    // Cache bounding sphere radius for AR fitting
    // this.geometry.computeBoundingSphere()
    // this.particlesRadius = (this.geometry.boundingSphere && this.geometry.boundingSphere.radius) || 1.0
    ...
    const onXRSessionEnded = () => {
      ...
      // Remove AR gesture listeners and reset state
    ...
    const placeParticlesForAR = () => {
      // Position sphere at fixed world coordinates relative to AR room
      // The sphere center is defined in this.arSphereCenter (x, y, z in meters)
      // Apply zoom factor to the Z distance for pinch-to-zoom functionality
      
      const center = this.arSphereCenter
      const zoomFactor = this.arZoomFactor || 1.0
      
      // Calculate final position with zoom applied to Z distance
      ...
      this.particles.position.copy(targetPos)
    }
    ...
    const startSession = async (mode) => {
      ...
      try {
        ...
        if (mode === 'immersive-ar') {
          ...
          // Place the particle cloud with computed distance and offset
          try {
            // Ensure particles are part of the scene graph (not attached to camera)
            if (this.particles.parent !== scene) scene.add(this.particles)
            placeParticlesForAR()
          } catch (_) {}
          // Enable pinch-to-zoom controls in AR
          this._onTouchStart = (e) => {
            if (e.touches && e.touches.length === 2) {
              ...
              this._pinch.startZoom = this.arZoomFactor
          ...
          this._onTouchMove = (e) => {
            if (this._pinch.active && e.touches && e.touches.length === 2) {
              ...
              const scale = (this._pinch.startDist > 0) ? (this._pinch.startDist / cur) : 1.0
              const nextZoom = this._pinch.startZoom * scale
              this.arZoomFactor = Math.min(3.0, Math.max(0.4, nextZoom))
            }
          }
          this._onTouchEnd = (e) => {
            if (!e.touches || e.touches.length < 2) {
        ...
        this.xrMode = mode === 'immersive-ar' ? 'ar' : 'vr'
      } catch (e) {
  ...
  updateScene(interval, elapsed) {
    ...
    if (renderer.xr.isPresenting && this.xrMode === 'ar') {
      // Recompute placement in case FOV changes as AR session stabilizes
      try { placeParticlesForAR() } catch (_) {}
    }
